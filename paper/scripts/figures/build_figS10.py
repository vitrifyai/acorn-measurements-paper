...
SRC = RAW_SEM_DIR
H=BUILD_DIR
OUT=BUILD_DIR/"build/si/S10"; OUT.mkdir(parents=True,exist_ok=True)
...
fc={r["stem"]:r for r in csv.DictReader(open(WORK_DIR / "additional_info/results/sem/field_characterisation.csv"))}
def ev(sub):
    sub_ = "sem_strat" if sub == "sem_stratified" else sub
    o=json.load(open(WORK_DIR / f"additional_info/results/{sub_}/heldout_evaluation.json"))
    return o["across_seeds"]["f1"], o["across_seeds_confident_reference"]["f1"]
def perfield(sub):
    b={}
    sub_ = "sem_strat" if sub == "sem_stratified" else sub
    for r in csv.DictReader(open(WORK_DIR / f"additional_info/results/{sub_}/heldout_per_field.csv")):
...
ax.set_ylabel("held-out F1"); ax.set_ylim(0,0.95)
# legend above the axes: inside it sat over the bars
ax.legend(frameon=False,fontsize=5.2,ncol=2,loc="lower center",
...
# panel letters: 7 pt bold, upper-left of each axes, in figure coordinates so
# they sit just outside the plotting region and cannot cover data
for i, ax_ in enumerate(fig.axes):
    bb = ax_.get_position()
    fig.text(bb.x0 - 0.022, bb.y1 + 0.012, chr(ord("A") + i),
             fontsize=7, fontweight="bold", ha="left", va="bottom", color="black")
for e in ("pdf","png"):
